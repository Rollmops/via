 ...
 *
 * $Id: vcanny2d.c 3177 2008-04-01 14:47:24Z karstenm $
 *
 *****************************************************************/

/*! \brief vcanny2d -- 2D edge detection
...
int main (int argc, char **argv)
{
  ...
  VImage gradr=NULL,gradc=NULL;
  VAttrListPosn posn;
  char prg[50];	
  sprintf(prg,"vcanny2d V%s", getVersion());
  fprintf (stderr, "%s\n", prg);
  ...
  for (VFirstAttr (list, & posn); VAttrExists (& posn); VNextAttr (& posn)) {
    ...
    VCanny2d (src,(int)dim,&gradr,&gradc);
    xdest = VMagnitude2d(gradr,gradc,NULL);
