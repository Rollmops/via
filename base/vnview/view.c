...
static void SelectBandCB (Widget w, XtPointer cl_data, XtPointer ca_data)
{
    ...
    /* geaendert: G.Lohmann, 15.12.04 */
    /* view->band = cb->value */
    view->band = cb->value * VImageNColors(view->image);
    if (view->band >= VImageNBands(view->image)) return;
...
void UpdateView (View view, int changes)
{
	...
	} else { /* !!! geandert, G.Lohmann, 15.12.04, Frames statt Bands ! */
	    XtVaSetValues (view->band_scale,
			   XmNmaximum, VImageNFrames(view->image) - 1,
			   XmNvalue, view->band, (char *) NULL);
	    
	    sprintf (buf, "%d", VImageNFrames(view->image) - 1);
	    str = XmStringCreateSimple (buf);
...
static void UpdatePixelReport (View view, int x, int y)
{
  ...
  if (VImageViewWindowToImage (view->image_view, x, y, & row, & column)) {
	...
	if (repn >= VBitRepn && repn <= VLongRepn)
	  ...
	  sprintf (buf, " %d,%d: %.3f", r, c,
		   VGetPixel (view->image, view->band, r, c));
      }
    } 
    else sprintf (buf, " %3d,%3d:", r, c);
  } 
  else buf[0] = 0;
  str = XmStringCreateSimple (buf);
